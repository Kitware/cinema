(function () {
    ...
    var getSharedData = function (model, container, analysis) {
        ...
        if (_.has(sharedDataMap, key)) {
            ...
            var layer = new cinema.models.LayerModel(model.getDefaultPipelineSetup(), {
                ...
                renderView = new cinema.views.VisualizationWebGlLightCanvasWidget({
                ...
                offScreenRenderView = new cinema.views.VisualizationWebGlLightCanvasWidget({
                    el: $('.c-body-container', container),
                    model: model,
    ...
    cinema.views.CompositeWebGlLightView = Backbone.View.extend({
    ...
    cinema.views.CompositeWebGlLightSearchView = Backbone.View.extend({
        initialize: function (opts) {
            this.compositeModel = new cinema.decorators.Composite(this.model);
            this._hasAnalysis = _.has(this.model.get('metadata'), 'analysis');
            var sharedData = getSharedData(this.compositeModel, this.$el, this._hasAnalysis);
    ...
    cinema.viewMapper.registerView('composite-image-stack-litdepth', 'view', cinema.views.CompositeWebGlLightView, {
        controls: [
    ...
    cinema.viewMapper.registerView('composite-image-stack-litdepth', 'search', cinema.views.CompositeWebGlLightSearchView, {
        controls: [
            { position: 'left', key: 'information', icon: 'icon-help', title: 'Information' },
            { position: 'right', key: 'tools', icon: 'icon-tools', title: 'Tools' }
        ]
